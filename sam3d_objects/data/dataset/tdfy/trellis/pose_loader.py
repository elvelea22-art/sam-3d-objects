...
from sam3d_objects.data.dataset.tdfy.objaverse.utils import (
    blender2pytorch3d,
)
...
class R3:
    ...
    def load_pose(transforms_path: str, file_name: str = "rgba_001.png") -> dict:
        ...
        camera_K_ndc = R3.convert_camera_K_to_ndc(torch.tensor(camera_K)[:3,:3], image_size_hw)
        return {
            file_name: {
                **instance_pose,
                "camera_K": camera_K,
    ...
    def load_pointmap(image_basename: str, pointcloud_directory: str, sha256: str = None, image_fname: str = None) -> torch.Tensor:
        ...
        if os.path.exists(standard_dir):
            ...
            if not frame:
                raise ValueError(f"Could not extract frame number from image_fname: {image_fname}")
